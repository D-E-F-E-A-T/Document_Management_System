...
class Template
{
    public function loadHead($defaultCSS, $title)
    {
        ...
        $content .= $this->addJS(
            false,
            "https://cdnjs.cloudflare.com/ajax/libs/tinymce/5.1.2/tinymce.min.js"
        );
        $content .= '<style>';
        $content .=
            '.btn-primary { color: #fff; background-color: #' .
            $colorcode .
            '; border-color: #' .
            $colorcode .
            '; }';
        $content .=
            '.btn-primary:hover, .btn-primary:focus { color: #' .
            $colorcode .
        ...
        $content .=
            ...
            '; }';
        $content .=
            '.btn-outline-primary:hover, .btn-primary:focus, .dropdown-item:hover { color: #fff; background-color: #' .
            $colorcode .
            '; border-color: #' .
            $colorcode .
            '; }';
        $content .= '</style>';
        $content .=
            '<script>tinymce.init({ selector: "textarea#note", branding: false, height: "500" });</script>';
    ...
    public function loadheader()
    {
        ...
        $content .=
            ...
            ';">';
        $content .=
            '<span class="navbar-brand mb-0 h1"><img src="' .
            $path->getPath("assets") .
            'img/icon.svg" width="30" height="30" class="d-inline-block align-top" alt=""> ' .
            LANG_title .
            '</span>';
        $content .=
        ...
        $content .= $bootstrap->addNavigationItem(
            "dashboard",
            LANG_NAV_dashboard
        );
        $content .= $bootstrap->addNavigationItem(
        ...
        $content .=
            ...
            $_SESSION["dms_user_realname"] .
            '</a>';
        $content .=
            '<div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">';
        $content .= $bootstrap->addDropdownItem(
            "index.php?page=profile&action=view",
            LANG_NAV_profile
        );
        $content .= $bootstrap->addDropdownItem(
    ...
    {
        global $path;
        return '<link rel="icon" type="image/png" sizes="' .
            $size .
            'x' .
            $size .
            '" href="' .
    ...
    {
        global $path;
        return '<link rel="apple-touch-icon" sizes="' .
            $size .
            'x' .
            $size .
            '" href="' .
            $path->getPath("assets") .
            'img/apple/apple-touch-icon.png">';
    }

    private function addAppleIconPre($size)
    {
        global $path;
        return '<link rel="apple-touch-icon-precomposed" sizes="' .
            $size .
            'x' .
            $size .
            '" href="' .
            $path->getPath("assets") .
            'img/apple/apple-touch-icon-precomposed.png">';
    }

    private function addAppleSafariIcon($color)
    {
        global $path;
        return '<link rel="mask-icon" href="' .
            $path->getPath("assets") .
            'img/apple/safari-pinned-tab.svg" color="' .
            $color .
            '">';
    }
