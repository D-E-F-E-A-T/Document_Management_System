...
class User
{
    ...
    public function doSignIn($username, $password)
    {
        ...
        $queryLogin = $mysql->query(
            "SELECT id,username,password FROM dms_users WHERE username = '" .
                $this->username .
                "' AND password = '" .
                $this->password .
                "'"
    ...
    public function loginCorrect()
    {
        ...
        $this->setSession(
            "dms_user_realname",
            $this->getUserRealName($this->username)
        );
    ...
    private function getUserRealName($username)
    {
        global $mysql;
        $query = $mysql->query(
            "SELECT id,firstname,surname FROM dms_users WHERE username = '{$username}'"
        );
    ...
    public function logActivity($username)
    {
        ...
        $mysql->query(
            "INSERT INTO dms_log_signin (ip, useragent, username, dt_last) VALUES ('{$logIP}', '{$logUserAgent}', '{$logUsername}', '{$logDateTime}')"
        );
    }
}
?>
